...
def compute_dist(map):
    ...
    all_indexes = np.transpose(
        np.unravel_index(np.arange(dist_map.size), dist_map.shape)
    )
    all_neightboors = np.clip(
        all_indexes[:, None] + directions[None, :, :],
        0,
        np.array(dist_map.shape)[None, None] - 1,
    )
    ...
    while not np.all(dist_map < np.inf):
        ...
        selector = np.logical_and(
            (dist_map < np.inf).ravel(),
            (
                dist_map[all_neightboors[:, :, 0], all_neightboors[:, :, 1]] == np.inf
            ).any(-1),
        )
        ...
        is_valid = (
            (delta <= 1)
            + (height[:, 1:] == start_symbol)
            + (height[:, 1:] == (ord("z") - ord("a")))
        ) * valid_neightboor[:, 1:]
        assert is_valid.any()
        # Mark distance of valid neightboors
        indexes_valid = indexes[
            np.broadcast_to(np.arange(indexes.shape[0])[:, None], is_valid.shape)[
                is_valid
            ],
            :,
        ]
        base_dist = dist_map[indexes_valid[:, 0], indexes_valid[:, 1]]
        selected = neightboors[:, 1:, :][
            np.broadcast_to(is_valid[:, :, None], neightboors[:, 1:, :].shape)
        ].reshape((-1, 2))
        ...
        previous_dist_values = dist_map.ravel()[
            np.ravel_multi_index(selected_unique.T, map.shape)
        ]
